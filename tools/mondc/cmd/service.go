...
var serviceCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		f, _ = os.Create(fmt.Sprintf("%s/cms/server.go", folderPath))
		...
		f.Write([]byte(fmt.Sprintf(`appId: %s port: %d`, AppId, Port)))
		...
		err = handlerHookTemplate.Execute(buffer, map[string]string{
			"AppId":      AppId,
			"FolderPath": folderPath,
		})
		utils.MustNil(err)
